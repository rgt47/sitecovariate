...
\textbf{Background.} Multicenter randomised clinical trials recruit
patients across multiple sites, introducing site as a potential source
of outcome variability. Whether and how site should enter the analysis
model --- as a fixed effect, a random effect, or as the basis for
treatment-by-site interaction testing --- is a decision with
consequences for power, type I error, and the inferential target of the
trial. Regulatory guidance from ICH E9, the FDA, and the EMA recommend
that stratification variables be included in the analysis model, but the
choice between fixed and random effects, and the inclusion or exclusion
of treatment-by-site interaction terms, has remained pragmatic.

\textbf{Methods.} We conduct a Monte Carlo simulation comparing four
linear-mixed analysis models (no site, site as fixed effect, site as
random effect, site-by-treatment interaction) across four scenarios
crossing equal-versus-unequal site sizes with the
presence-versus-absence of true treatment-by-site interaction. Each
scenario is evaluated at \(n_{\text{sim}} = 1{,}000\) replicates with
bias, empirical SE, model-based SE, power, coverage, and convergence
rate reported.

\textbf{Results.} Including site as a random effect generally performs
as well as or better than the fixed-effects approach across all
scenarios examined, consistent
with\textsuperscript{\citeproc{ref-Kahan2013continuous}{1}}
and\textsuperscript{\citeproc{ref-edgarIncludingRandomCentre2021}{2}}.
The random-effects model avoids the loss of degrees of freedom inherent
in the fixed-effects approach, which becomes particularly problematic
when the number of sites is large relative to the total sample size, and
is amplified when site sizes are unequal. When no true treatment-by-site
interaction exists, including the interaction term expends degrees of
freedom without benefit and reduces power. Ignoring site when it is a
true source of outcome variability inflates residual variance and
reduces power.

\textbf{Conclusions.} Site should be included in the analysis model as a
random effect when site is a stratification variable or contributes to
outcome variability. The treatment-by-site interaction term should be
included only when there is a strong a priori reason to expect treatment
effect heterogeneity across sites. The simulation evidence supports the
regulatory recommendation that stratification variables enter the
...
power\textsuperscript{\citeproc{ref-Kahan2013continuous}{1},\citeproc{ref-Localio2001adjustments}{3}}.
...
methods\textsuperscript{\citeproc{ref-Localio2001adjustments}{3}}.
...
analysis'\textsuperscript{\citeproc{ref-ICHE9guideline}{4}}. When
...
model\textsuperscript{\citeproc{ref-FDA2023covariate}{5}}. The European
...
administrative
reasons\textsuperscript{\citeproc{ref-EMA2015covariate}{6}}.
...
power\textsuperscript{\citeproc{ref-Kahan2014rewards}{7}}. When outcome
...
the treatment
effect\textsuperscript{\citeproc{ref-Kahan2013continuous}{1},\citeproc{ref-edgarIncludingRandomCentre2021}{2}}.
...
\textsuperscript{\citeproc{ref-Kahan2013continuous}{1}} conducted a
...
precisely\textsuperscript{\citeproc{ref-edgarIncludingRandomCentre2021}{2}}.
...
problem\textsuperscript{\citeproc{ref-kimAnalysisMulticenterClinical2020}{12},\citeproc{ref-Kahan2015fewevents}{16}}.\textsuperscript{\citeproc{ref-Localio2001adjustments}{3}}
...
\texttt{lme4} package\textsuperscript{\citeproc{ref-R-lme4}{18}}:
...
Across four scenarios crossing balanced vs imbalanced site sizes with
and without true treatment-by-site heterogeneity, all four analysis
models produced absolute bias below 0.002 (worst case: no\_site under
``Equal, with interaction'', MCSE 0.002). Coverage of 95\% confidence
intervals across all combinations fell in {[}0.914, 0.983{]}. Mean power
by model (averaged across scenarios) was: no\_site = 0.94, site\_fixed =
0.97, site\_random = 0.97, site\_x\_trt = 0.97. All performance measures
are reported with their Morris Table 6 Monte Carlo SEs in the
per-scenario tables below.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Bias of the treatment effect estimate across four models and four simulation scenarios.}]{report_files/report_files/figure-latex/fig-bias-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Power to detect the treatment effect across four models and four simulation scenarios.}]{report_files/report_files/figure-latex/fig-power-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Coverage of nominal 95 percent confidence intervals across four models and four simulation scenarios. The dashed line indicates the nominal 95 percent level.}]{report_files/report_files/figure-latex/fig-coverage-1.pdf}}
...
of\textsuperscript{\citeproc{ref-Kahan2013continuous}{1}}
and\textsuperscript{\citeproc{ref-edgarIncludingRandomCentre2021}{2}},
...
model\textsuperscript{\citeproc{ref-ICHE9guideline}{4}--\citeproc{ref-EMA2015covariate}{6}}.
...
contribute sparse
data\textsuperscript{\citeproc{ref-Localio2001adjustments}{3},\citeproc{ref-Agresti2000strategies}{14}}.
...
populate the manuscript's tables and figures are openly available in the
\texttt{sitecovariate} project repository. Specific paths are listed in
the Reproducibility section below.

\section{Reproducibility}\label{reproducibility}

This manuscript was rendered with R 4.4.x. Principal packages used by
the simulation pipeline are \texttt{lme4} (the linear-mixed fitter),
\texttt{MASS} (multivariate normal data generation), \texttt{furrr} and
\texttt{future} (parallel processing), \texttt{dplyr} and
\texttt{ggplot2} (reporting), and \texttt{rmarkdown} plus
\texttt{bookdown} for the manuscript build.

\textbf{Random-number generator.} The simulation driver
follows\textsuperscript{\citeproc{ref-morris2019using}{17}} §4.1:
\texttt{RNGkind} is pinned and \texttt{set.seed(20260411)} is called
once at the start of the program. Per-replicate seeds derive from this
master stream.
...
\emph{Source:
\texttt{\textasciitilde{}/prj/res/11-site-covariate-analysis/sitecovariate/analysis/report/report.Rmd}}
...
\CSLLeftMargin{18. }%
\CSLRightInline{Bates D, Maechler M, Bolker B, Walker S. \emph{Lme4:
Linear Mixed-Effects Models Using Eigen and S4}.; 2024.}
